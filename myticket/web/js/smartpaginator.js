    ...
    $.fn.extend({
        smartpaginator: function (options) {
            ...
            return this.each(function () {
                ...
                var list = $('<ul/>');
                var btnPrev = $('<div/>').text(settings.prev).click(function () { if ($(this).hasClass('disabled')) return false; currentPage = parseInt(list.find('li a.active').text()) - 1; navigate(--currentPage); });
                var btnNext = $('<div/>').text(settings.next).click(function () { if ($(this).hasClass('disabled')) return false; currentPage = parseInt(list.find('li a.active').text()); navigate(currentPage); });
                var btnFirst = $('<div/>').text(settings.first).click(function () { if ($(this).hasClass('disabled')) return false; currentPage = 0; navigate(0); });
                var btnLast = $('<div/>').text(settings.last).click(function () { if ($(this).hasClass('disabled')) return false; currentPage = totalpages - 1; navigate(currentPage); });
               ...
               // var btnGo = $('<input/>').attr('type', 'button').attr('value', settings.go).addClass('btn').click(function () { if (inputPage.val() == '') return false; else { currentPage = parseInt(inputPage.val()) - 1; navigate(currentPage); } });
                //var info=$('<div/>').text('dctuj');
                var info='';
                if(settings.datacontainer=='table-orders')
                   info='Кол-во всего: '+settings.totalrecords+'(из них Отмен:'+settings.voided+')';
                if(settings.totalrecords>0)
                container.append(list).append($('<div/>').addClass('short').text(info));
               // else
                // container.append($('<div/>').addClass('short').text(info));
